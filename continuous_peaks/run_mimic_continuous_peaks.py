...
T_PCT = yaml.load(open('parameters.yml'), yaml.Loader)['t_pct']
...
EXPERIMENT_NAME = f'ContinuousPeaks_{PROBLEM_SIZE_LIST[0]}_MIMIC'
            ...
            for population_size in POPULATION_SIZES_LIST:
                ...
                for keep_percent in KEEP_PERCENT_LIST:
                    ...
                    for i in tqdm(range(NUM_RUNS)):
                        problem = ContinuousPeaksGenerator().generate(seed=SEED+i, size=problem_size, t_pct=T_PCT)
                        sa = MIMICRunner(
                            problem=problem,
